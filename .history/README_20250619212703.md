...
- Fetching and displaying cards from both local data and a remote API, merging them seamlessly.
- Adding, liking/unliking, and deleting cards with real-time updates synced with the backend.
- User profile management with editable name, description, and avatar — including uploading a new profile picture with server integration.
- Modular, reusable components with classes like Card, UserInfo, and various popup handlers for forms and image previews.
- Robust form validation, asynchronous API interactions, and smooth user experience with loading states and error handling.
