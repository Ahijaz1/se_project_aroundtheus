    ...
    cardSection = new Section(
      {
        items: initialCards,
        renderer: (item) => {
          const card = createCard(item); // now returns Card instance
          const cardElement = card.getView(); // get the element
          cardSection.addItem(cardElement);
          return card; // optionally return card instance if needed
        },
      },
      ".cards__list"
    );
...
const deleteCardPopup = new PopupWithSubmit("#delete-card-modal", () => {
  ...
  if (cardId) {
    ...
    // Local card: just remove from DOM without API call
    cardToDelete.remove();
    deleteCardPopup.close();
    cardToDelete = null;
    deleteSubmitButton.textContent = "Save";
...
function createCard(cardData) {
  ...
  return card; // returns the Card instance (object)
}
