...
function handleLikeClick(card) {
  ...
  if (isLiked) {
    api
      ...
      .then((updatedCard) => {
        const userHasLiked = updatedCard.likes.some(
          (user) => user._id === currentUserId
        );
        card.setLikeStatus(userHasLiked);
        // Optional: update like count if you track it
        // card.updateLikeCount(updatedCard.likes.length);
      })
      .catch((err) => console.error(err));
  } else {
    api
