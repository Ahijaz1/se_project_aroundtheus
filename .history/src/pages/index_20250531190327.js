...
Promise.all([api.getUserInfo(), api.getInitialCards()])
  .then(([userData, cards]) => {
    currentUserId = userData._id; // store this for like checking //
    ...
    currentUserId = new Section(
      {
        items: cards,
        renderer: (item) => {
          const card = createCard(item, currentUserId);
          return card; // Just return the element //
        },
      },
      ".cards__list"
    );

    currentUserId.renderItems();
  })
...
function createCard(cardData, userId) {
  ...
  const card = new Card(
    { ...cardData, isLiked }, // pass new computed value
    "#card-template",
    handleImageClick,
